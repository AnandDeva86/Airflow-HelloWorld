...
class TestDagValidation:
    ...
    def test_import_dags(self, dagbag):
        """
            ...
            - check for cycles
        """
        assert len(dagbag.import_errors) == 0, "DAG failures detected! Got: {}".format(dagbag.import_errors)
    ...
    def test_number_of_dags(self, dagbag):
        ...
        assert dag_num == self.EXPECTED_NUMBER_OF_DAGS, \
            "Wrong number of dags, {0} expected got {1} (Can be due to cycles!)" \
