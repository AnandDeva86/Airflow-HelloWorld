...
def dag(dagbag):
    return dagbag.get_dag('projectA')


class TestTstDagDefinition:
    EXPECTED_NB_TASKS = 2
    EXPECTED_TASKS = ['bash_task_1', 'bash_task_2']
    ...
    def test_nb_tasks(self, dag):
        ...
        assert nb_tasks == self.EXPECTED_NB_TASKS, \
            "Wrong number of tasks, {0} expected, got {1}".format(self.EXPECTED_NB_TASKS, nb_tasks)
    ...
    def test_dependencies_of_tasks(self, dag, task, expected_upstream, expected_downstream):
        ...
        assert self.compare(task.upstream_task_ids,
                            expected_upstream), "The task {0} doesn't have the expected upstream dependencies".format(
            task)
        assert self.compare(task.downstream_task_ids,
                            expected_downstream), "The task {0} doesn't have the expected downstream dependencies".format(
            task)
    ...
    def test_start_date(self, dag):
        """
        Verify that the start_date is < current date
        """
        assert dag.default_args['start_date'] < pendulum.now()
